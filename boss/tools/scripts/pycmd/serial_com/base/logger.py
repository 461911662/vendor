...
import datetime
import os
from typing import AnyStr, BinaryIO, Callable, Optional
...
class Logger:
    def __init__(
       self, elf_files, console, timestamps, timestamp_format, realtime=True,
    ):
    ...
    def start_logging(self):  # type: () -> None
        if not self._log_file:
            if self.elf_file:
                ...
                name = 'log.{}.txt'.format(datetime.datetime.now().strftime('%Y%m%d%H%M%S'))
