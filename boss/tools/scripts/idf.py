...
import subprocess
import sys
from typing import Any, Callable, Counter, Dict, List, Optional, OrderedDict, Union

try:
    from pycmd.errors import FatalError
    from pycmd.utils import fatal, warn, info
    from pycmd.idf_py_actions.tools import merge_action_lists, get_target, PropertyDict
    ...
    print((f'Cannot import module "{e.name}". This usually means that "idf.py" was not '
           f'spawned within an FREE-IDF shell environment or the python virtual '
           f'environment used by "idf.py" is corrupted.\n'
           f'Please use idf.py only in an FREE-IDF shell environment. If problem persists, '
           f'please try to install FREE-IDF tools again as described in the Get Started guide.'),
          file=sys.stderr)
...
# custom action's directory
CUSTOM_ACTION_PATH_PROMPT = 'IDF_EXTRA_ACTIONS_PATH'

def _check_environment() -> List:
    """
    ...
    if 'IDF_PATH' in os.environ:
        ...
        warn('Setting IDF_PATH environment variable: %s' % detected_idf_path)
...
def init_cli(verbose_output: List=None) -> Any:
        ...
        for option in ctx.command.params:
            ...
            if isinstance(option, Option) and option.deprecated and ctx.params[option.name] != default:
                ...
                if deprecation.exit_with_error:
                    ...
                    warn('Warning: %s' % deprecation.full_message('Option "%s"' % option.name))
            ...
            if not global_args.dry_run:
                for task in tasks_to_run.values():
                    ...
                    if name_with_aliases != 'help' or not task.action_args.get('json_option', False):
                        info('Executing action: %s' % name_with_aliases)
    ...
    idf_py_extensions_path = os.path.join(os.environ['IDF_PATH'], 'scripts', 'pycmd', 'idf_py_actions')
    extension_dirs = [os.path.realpath(idf_py_extensions_path)]
    extra_paths = os.environ.get(CUSTOM_ACTION_PATH_PROMPT)
    ...
    if os.path.exists(os.path.join(project_dir, 'idf_ext.py')):
        ...
        try:
            ...
            warn('Error importing extension file idf_ext.py. Skipping.')
            warn("Please make sure that it contains implementation (even if it's empty) of add_action_extensions")
    ...
    cli_help = (
        'FREE-IDF CLI build management tool. '
...
def main(argv: List[Any] = None) -> None:
    ...
    checks_output = None if SHELL_COMPLETE_RUN else _check_environment()
